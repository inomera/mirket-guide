...
Mirket, kapsamlı bir API yönetimi ürünüdür. Organizasyonların web uygulamaları için programlama arayüzlerini (API'ler) kolaylıkla oluşturmasını, yayınlamasını ve yönetmesini sağlar. Mirket ile API'lerinizin kullanım politikalarını uygulayabilir, erişimi kontrol edebilir, kullanıcı topluluğunuzu geliştirebilir, kullanım istatistiklerini toplayıp analiz edebilir ve performans raporları oluşturabilirsiniz.

Modern mikroservis mimarilerinde Mirket API Gateway, istemciler ile backend servisleri/mikroservisler arasında köprü görevi görür. Temel olarak API Gateway, gelen istekleri alıp çeşitli kurallara göre uygun servislere yönlendiren gelişmiş bir ters vekil sunucusu (reverse proxy) olarak çalışır.

== Versiyon Yönetimi

Mirket, yazılım geliştirme süreçlerinde yaygın olarak kullanılan semantik versiyonlama yöntemini benimsemektedir:

* **Major (X.0.0):** Önemli yapısal değişiklikler içerir ve geriye dönük uyumluluk garanti edilmez
* **Minor (1.X.0):** Yeni özellikler ekler, geriye dönük uyumluluk korunur
* **Patch (1.0.X):** Hata düzeltmeleri ve küçük iyileştirmeler içerir, geriye dönük uyumluluk korunur

Tüm versiyon güncellemeleri öncesinde kapsamlı sürüm notları paylaşılmaktadır. Minor ve Patch güncellemelerinde geriye dönük uyumluluk sağlanırken, Major güncellemelerde yapısal değişiklikler nedeniyle geriye dönük uyumluluk garanti edilmemektedir.
